...
1. 가로 길이가 v일 때, 맨 처음(idx=0)과 맨 끝(idx=v-1)에서는 고인 빗물의 양을 구할 수 없다. 
2. i번째 블록을 기준으로 왼쪽, 오른쪽 벽에 의해 공간이 만들어져야 한다.
3. 빗물이 고이는 양은 i번째 블록(자기 자신)을 기준으로 왼쪽에서(idx:0~i-1)가장 큰 블록과 오른쪽에서(idx:i+1~v-1) 가장 큰 블록에 의해 결정된다.
4. 그래서 이 둘을 먼저 구해야 한다.
5. 그런 다음에 이 둘 중 더 작은 값에서 i번째 블록의 높이를 빼면 i번째에 고인 빗물의 양을 구할 수 있다.
6. 단, 이때 구한 값이 음수이면 i번째 블록에는 빗물이 고이지 않은 것이므로 무시하고 넘어간다.
7. 이렇게 구한 값을 모두 더하면 빗물의 총량을 구할 수 있다.
