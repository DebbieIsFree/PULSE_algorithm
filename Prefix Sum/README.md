...
<span style="color:blue">1 문제 조건</span>
...
<span style="color:blue">2 문제 풀이</span> 
 1) 300 * 300인 2차원 배열을 만들어 총이익을 입력받는다.
 2) 시작점부터 임의의 점까지 누적합을 구한다.
 3) 누적합으로부터  시작점이 (a,b)이고 끝점이 (x,y)인 구간합을 구한다.   ★ 이때 (a,b)부터 (x,y)까지의 구간이 k &#42; k인 정사각형 형태여야 한다. ( k의 범위 : 1 ~ N )

-----------------------------------------------------------------------

<span style="color:blue">3 소스 코드</span>
...
* 단, (시작점 + K) 즉, (a+k, b+k)가 배열의 범위 (N,N)을 넘지 않도록 범위 검사 
...
* 모든 경우를 다 탐색하는 완전탐색
* 모든 경우를 하나씩 고려하고, 범위 안에 있을 경우에는 최대값이 되는지 확인 후 된다면 계속해서 . 
